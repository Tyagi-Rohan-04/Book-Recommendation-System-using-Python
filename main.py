...
def search():
    ...
    if request.method == 'GET':
        ...
        if response.status_code==200:           # 200 means the request was successful, 404 means not found, 401 means unauthorized
            ...
            print("Failed to fetch the data from Google Books API.")
            messagebox.showinfo("Info", "Failed to fetch the data from Google Books API.")
...
Button(root, image=Logout_image, bg="#0099ff", cursor="hand1", command=lambda: root.destroy()).place(x=1200, y=20)
